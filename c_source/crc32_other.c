 ...
 * =====================================================================================
 *
 *       Filename:  crc32_other.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  2020년 04월 13일 14시 14분 42초
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int create_file_info(FILE *fp, const char *path)
{
	...
	while((file = readdir(dir_ptr)) != NULL) {
		...
		/* 검색된 파일이 directory이면 재귀호출로 하위 디렉토리를 다시 검색 */
			...
			// 검색된 이름의 속성이 일반파일이면
			/* 파일 정보를 CSV 형태로 출력함 * 파일명, path/파일명, 파일크기, CRC32값 */
...
int main(int argc, char **argv)
{
	...
	crc32 = get_file_crc32(argv[1]);
	printf("[%s:%d] crc32(%s) = %u\n", __FILE__, __LINE__, argv[1], crc32);
#else
	FILE *fp = stdout;
	if(argc == 1) {

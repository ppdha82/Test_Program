...
static long U[SIZE], V[SIZE], Y1[SIZE], Y2[SIZE];
...
/*
//At the time of conversion, it is converted last in the buffer, RGB is aligned with B, G, R, the converted image is upside down, and you can write the converted buffer directly to the BMP file to view it normally.
...
static long int crv_tab[SIZE];   
static long int cbu_tab[SIZE];   
static long int cgu_tab[SIZE];   
static long int cgv_tab[SIZE];   
static long int tab_76309[SIZE]; 
...
memory structure
            ...
            +----------+
                w/2
 */
#if defined(_SEPERATE_YUV_)
void yuv420p_to_rgb24(UINT8* yData, UINT8* uData, UINT8* vData,unsigned char* rgbbuffer, int width,int height)
#else
void yuv420p_to_rgb24(unsigned char* yuvbuffer,unsigned char* rgbbuffer, int width,int height)
#endif  /* _SEPERATE_YUV_ */
{
    int y1, y2, u, v;    
...
int main(int argc, char** argv)
{
	...
	const char* OUTPUT_FILE = "nalbi_dump_1_t3.rgb";
...
#else
    UINT8 *yuv420spData = (UINT8*)malloc(pic_size * 3 / 2);
    UINT8 *yuv420pData = (UINT8*)malloc(pic_size * 3 / 2);
#endif  /* _SEPERATE_YUV_ */
    ...
    yuv420p_to_rgb24(y_data, u_data, v_data, buffer, WIDTH, HEIGHT);
#else
    fread(yuv420spData, 1, pic_size * 3 / 2, in);
    yuv420sp_to_yuv420p(yuv420spData, yuv420pData, WIDTH, HEIGHT);
    yuv420p_to_rgb24(yuv420pData, buffer, WIDTH, HEIGHT);
#endif  /* _SEPERATE_YUV_ */
